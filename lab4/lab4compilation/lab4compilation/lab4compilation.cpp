...
public:
    ...
    void processVarDecl(shared_ptr<ASTNode> node)
    {
    ...
    string getExprType(shared_ptr<ASTNode> node)
    {
        ...
        if (node->name == "Identifier")
        {
            ...
            {
                errors.push_back(
                    "Необъявленная переменная: " + node->value
                );
                return "error";
            }
        ...
        {
            string left =
                getExprType(node->children[0]);

            string right =
                getExprType(node->children[1]);
    ...
    string generateExpr(shared_ptr<ASTNode> node)
    {
        if (node->name == "Identifier")
            return node->value;
        ...
        if (node->name == "BinaryExpr")
        {
            ...
            return "^" +
                to_string(triads.size());
        }

        return "";
    }

    void processAssign(shared_ptr<ASTNode> node)
    {
        string leftName =
            node->children[0]->value;

        if (!symbols.count(leftName))
        {
            errors.push_back(
        ...
        symbols[leftName].initialized = true;
    }
    ...
    void printSymbols()
    {
        ...
        cout
            << "Имя\tТип\tОбъявлена\tИнициализирована\n";
